 ...
 *   `migrations/*.sql`. Idempotent — oncurrent callers share one promise.
...
if (typeof window === "undefined") {
  void ensureDbReady();
  // Void intentional: don't block module evaluation on rejection. The next
  // getSql() call will retry if this pass failed.
}
